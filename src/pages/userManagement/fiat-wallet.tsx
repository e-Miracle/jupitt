...
import FiatCard from "../../components/fiat-card.tsx";
    ...
    <>
      <Box>
        ...
        <Box className="mt-5">
          ...
          >
            {user &&
              user?.fiat_wallets.length > 0 &&
              user?.fiat_wallets.map((item) => (
                <FiatCard
                  name={item.bank_name}
                  key={item.id}
                  country={item.account_name}
                  value={item.balance}
                  currency={item.account_name}
                  onOpenCredit={() => {
                    setType(item.bank_name);
                    onModal1Open();
                  }}
                  onOpenDebit={() => {
                    setType(item.bank_name);
